 ...
 * @param x 横坐标
 * @param y 纵坐标
 * @param mode 模式 默认值: 2(增强型) 其他值: 1(原始型)
 * @param mFactor 放大因子 默认值:2(200dpi 机器)/3(300dpi 机器) 其他值:1 到 10
 * @param ecclever 纠错率：默认值:Q(参数为空)/M(参数非法) 其他值:H = 超高纠错等级 Q = 高纠错等级 M = 普通纠错等级 L = 高密度等级
 * @param maskValue 掩码 默认值: 7 其他值: 0 到 7
 */
-(void)addQRCode:(NSString*) content x:(int)x y:(int)y mode:(int)mode mFactor:(int)mFactor ecclever:(NSString *)ecclever maskValue:(int)maskValue;
 ...
 * @param x 横坐标
 * @param y 纵坐标
 * @param orienftation 条码方向
 * @param barHeight 条码高度 (dot)
 * @param wh 条码宽窄比，默认2，范围 1 ～ 10
